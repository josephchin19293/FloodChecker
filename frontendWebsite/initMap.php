<?php
    ...
    $dbc = mysqli_connect("localhost","root","root","FloodChecker");
    //Get middle
    $query = "SELECT MIN(dataLatitude) as minLat , MAX(dataLatitude) as maxLat , Min(dataLongitude) as minLong , MAX(dataLongitude) as maxLong FROM Data";
    ...
    $query = "SELECT DISTINCT(sensor) as sensor FROM Data";
    $result = mysqli_query($dbc,$query) or die("Failed to query database");
    $sensors = [];
    ...
    for($i = 0; $i< count($sensors);$i++) {
        $reading = [];
        $query = "SELECT * FROM Data where sensor='".$sensors[$i] ."' ORDER BY dataId DESC";
        $result = mysqli_query($dbc,$query) or die("Failed to query database");
        ...
        $query = "SELECT * FROM Data where sensor='".$sensors[$i] ."' ORDER BY dataId ASC";
        $result = mysqli_query($dbc,$query) or die("Failed to query database");
        while($row = mysqli_fetch_array($result)) {
